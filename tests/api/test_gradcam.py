...
class TestGradCAM:
    ...
    def test_init_buffers(self, mock_model):
        ...
        assert gc.gradients is None and gc.activations is None

    def test_forward_backward_hooks(self, mock_model):
        """Store gradients and activations correctly through hooks."""
        gc = GradCAM(mock_model, mock_model.features.denseblock4)
        t = torch.randn(1, 512, 7, 7)
        gc.forward_hook(None, None, t)
        gc.backward_hook(None, None, (t,))
        assert gc.activations.shape == t.shape
        assert gc.gradients.shape == t.shape
    ...
    def test_cam_shape(self, mock_model):
        ...
        assert isinstance(cam, np.ndarray) and cam.shape == (7, 7)
