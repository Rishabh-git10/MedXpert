...
class TestPredictImage:
    ...
    def test_required_fields(self, sample_xray_bytes):
        ...
        for f in findings:
            assert {"name", "probability", "index"} <= set(f.keys())
    ...
    def test_no_finding_logic(self, sample_xray_bytes):
        ...
        findings, _ = predict_image(sample_xray_bytes)
        no_f = any(f["name"] == "No Finding" for f in findings)
        others = any(f["name"] != "No Finding" for f in findings)
        if others:
            assert not (no_f and len(findings) > 1)
    ...
    def test_grayscale_supported(self):
        ...
        findings, heatmaps = predict_image(buf.tobytes())
        assert isinstance(findings, list)
        assert isinstance(heatmaps, dict)
